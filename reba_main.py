...
class DualCameraCapture:
    ...
    def __init__(self, cam0_idx=0, cam1_idx=1, fps=30):
        ...
        self.ema_landmarks0 = None  # Smoothed landmark positions for cam 0
        ...
        if not self.cap0.isOpened():
            raise RuntimeError(f"Cannot open camera {cam0_idx}")
        if not self.cap1.isOpened():
            raise RuntimeError(f"Cannot open camera {cam1_idx}")
    ...
    def calculate_stability(
        ...
        for i, (curr, prev) in enumerate(zip(current_landmarks, previous_landmarks)):
            ...
            stability_scores.append(float(np.mean(stability_history[i])))
    ...
    def process_mediapipe_frame(
        ...
        if results.pose_landmarks:
            ...
            valid_count = sum(1 for lm in smoothed_landmarks if lm["visibility"] >= 0.5)
